...
class StorageFactory:
    ...
    def create(storage_type: str = None, **kwargs) -> StorageBackend:
        storage_type = (storage_type or os.getenv("DOC_UPLOAD_STORAGE", "local")).lower()

        if storage_type == "local":
            from pdf_autofillr_doc_upload.storage.local_storage import LocalStorage
            return LocalStorage(
                data_path=kwargs.get("data_path", os.getenv("DOC_UPLOAD_DATA_PATH", "./data/doc_upload")),
                config_path=kwargs.get("config_path", os.getenv("DOC_UPLOAD_CONFIG_PATH", "./configs")),
            )
