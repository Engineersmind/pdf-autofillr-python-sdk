...
class InProcessMapperFiller(PDFFillerInterface):
    ...
    def __init__(
        ...
        self.data_path = data_path or os.getenv("DOC_UPLOAD_DATA_PATH", "./data/doc_upload")
        ...
        self.filled_pdf_path: Optional[str] = None

    # ── PDFFillerInterface stubs (not used in local mode) ──────────────────

    def make_embed_file(self, user_id, pdf_doc_id, session_id, investor_type, use_second_mapper=True) -> bool:
        """Not used for in-process filling — prepare_and_fill() handles everything."""
        return True

    def check_embed_file(self, user_id, pdf_doc_id, investor_type, max_attempts=48, wait_interval=10):
    ...
    def prepare_and_fill(
        ...
        session_dir = Path(self.data_path) / "jobs" / job_id / "mapper"
        ...
        if output_path is None:
            ...
            out_dir = Path(self.data_path) / "output" / job_id
        ...
        if not Path(output_path).exists():
            ...
            candidates = list(session_dir.glob("*filled*.pdf")) + list(session_dir.glob("*.pdf"))
            if candidates:
    ...
    def _get_impl(self):
        if self._impl is not None:
            return self._impl
        try:
            from pdf_autofillr_mapper.inprocess_filler import InProcessMapperFiller as _Impl
