...
def run_pytest(label: str, args: list, env: dict | None = None) -> dict:
    ...
    for line in output.splitlines():
        ...
        if "passed" in line or "failed" in line or "error" in line or "skipped" in line:
            ...
            failed = int(f.group(1)) if f else failed

            s = re.search(r"(\d+) skipped", line)
            skipped = int(s.group(1)) if s else skipped

            e = re.search(r"(\d+) error", line)
            errors = int(e.group(1)) if e else errors
...
if not SCHEMA.exists():
    ...
    ok("configs/form_keys.json present")
...
if api_key and not api_key.startswith("sk-..."):
    ...
    warn(
        "DOC_UPLOAD_LLM_API_KEY not set in .env — real LLM and mapper e2e tests will be skipped"
    )
...
if pdf_path and Path(pdf_path).exists():
    ...
    warn(
        f"DOC_UPLOAD_PDF_PATH not found: {pdf_path!r} — mapper e2e test will be skipped"
    )
...
if java_check.returncode == 0:
    ...
    java_ver = (
        (java_check.stderr or java_check.stdout).splitlines()[0]
        if (java_check.stderr or java_check.stdout)
        else "found"
    )
...
if sys.platform == "win32":
    ...
    if Path("C:/Temp").exists():
        ...
        warn(
            "C:/Temp does not exist — create it: New-Item -ItemType Directory -Path C:\\Temp -Force"
        )
...
if not docx_path.exists():
    ...
    try:
        ...
        warn(
            "python-docx not installed — skipping docx creation (pip install python-docx)"
        )
...
for fname in [
    ...
    (
        ...
        if p.exists()
        else warn(f"data/input/{fname} MISSING — re-copy from repo")
    )
...
r = run_pytest(
    ...
    ["tests/functional/", "-v", "-k", "not real and not mapper_e2e"],
...
if txt_file.exists() and has_api_key:
    ...
    step(
        "Running: python -m entrypoints.local --document data/input/sample_investor.txt …"
    )
...
for r in results:
    ...
    print(
        f"  {r['label']:<40} {r['passed']:>7} {r['failed']:>7} {r['skipped']:>8} {status:>8}"
    )

print(f"  {'-'*40} {'-'*7} {'-'*7} {'-'*8} {'-'*8}")

print(
    f"  {'TOTAL':<40} {total_passed:>7} {total_failed:>7} {total_skipped:>8} {'✅ ALL OK' if all_ok else '❌ FAILURES':>8}"
)
