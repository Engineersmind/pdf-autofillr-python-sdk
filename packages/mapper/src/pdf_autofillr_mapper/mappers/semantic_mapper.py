...
class SemanticMapper:
    ...
    def __init__(
        ...
        from pdf_autofillr_mapper.core.config import settings, get_semantic_mapper_config, get_chunking_config
...
"""
    ...
    async def map(self, extracted_data: dict, input_schema: dict) -> dict:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='_extracted.json',
                                        delete=False, encoding='utf-8') as ef:
            import json
            json.dump(extracted_data, ef)
            extracted_path = ef.name

        with tempfile.NamedTemporaryFile(mode='w', suffix='_schema.json',
