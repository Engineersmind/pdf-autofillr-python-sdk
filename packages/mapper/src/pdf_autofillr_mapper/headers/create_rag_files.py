...
async def create_rag_api_files(
    ...
    with open(final_form_fields_path) as f:
        data = json.load(f)
...
def create_section_file(
    ...
    hierarchy_map: dict[Any, Any] = defaultdict(
    ...
    for h1, h2_dict in hierarchy_map.items():
        ...
        for h2, h3_dict in h2_dict.items():
            ...
            for h3, field_list in h3_dict.items():
                ...
                for field_info in field_list:
                    ...
                    if field_info["h4"]:
                        ...
                        h4_options.append(
                            {"h4": field_info["h4"], "field_id": field_info["field_id"]}
                        )
                    else:
                ...
                h3_field_entry = {"h3": h3, "field_id": h3_field_id}
            ...
            h2_sections.append(
                {"h2": h2, "section_context": section_context, "h3_fields": h3_fields}
            )

        sections.append({"h1": h1, "h2_sections": h2_sections})

    section_file: dict[Any, Any] = {
        "user_id": str(user_id),
        "session_id": session_id,
        "pdf_id": str(pdf_doc_id),
        "timestamp": datetime.datetime.utcnow().isoformat() + "Z",
