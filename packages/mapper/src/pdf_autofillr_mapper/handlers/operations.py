...
async def handle_extract_operation(
    ...
    try:
        ...
        extractor_config = {"WIDGET_LINE_DISTANCE_THRESHOLD": 10, "rounding": 1}
        ...
        storage_config = {"type": "local", "path": extraction_output_path}

        # Extract from PDF
        result = extractor.extract(pdf_path=local_pdf, storage_config=storage_config)

        # Save output immediately to source storage
        saved_path = output_handler.save_output(
            extraction_output_path, "extracted_json"
        )
        if saved_path:
...
async def handle_map_operation(
    ...
    try:
        ...
        if not local_mapping or not local_radio:
            ...
            raise ValueError(
                f"Config missing paths: local_mapped_json={local_mapping}, local_radio_json={local_radio}"
            )

        logger.info("Output paths configured:")
        logger.info(f"   Mapping: {local_mapping}")
        logger.info(f"   Radio groups: {local_radio}")

        storage_config = {"output_path": local_mapping, "radio_groups": local_radio}
        ...
        semantic_path = local_mapping.replace(
            "_mapped_fields.json", "_semantic_mapping.json"
        )
        ...
        for field_id, mapping_data in semantic_mappings.items():
            if isinstance(mapping_data, dict):
                ...
                java_mapping[field_id] = (
                    [field_name, "", confidence] if field_name else [None, None, 0]
                )
                ...
                logger.warning(
                    f"Field {field_id} has unexpected format: {mapping_data}"
                )
                java_mapping[field_id] = [None, None, 0]
        ...
        logger.info(
            f"✅ Converted {len(java_mapping)} fields to Java format -> {local_mapping}"
        )

        # Save outputs immediately to source storage
        # IMPORTANT: Save semantic mapping first (for cache), then Java format (for embedder)
        saved_semantic = output_handler.save_output(
            semantic_path, "semantic_mapping_json"
        )
        ...
        if notifier and NOTIFICATIONS_AVAILABLE:
            await safe_notify(
                ...
                output_files={"mapping": local_mapping, "radio_groups": local_radio},
                user_input_details=user_input_details,
                performance_metrics={
                    "total_fields_mapped": field_stats.get("total_fields_mapped", 0),
                    "high_confidence_count": field_stats.get(
...
async def handle_embed_operation(
    ...
    try:
        ...
        if not all([local_pdf, local_extracted, local_mapping, local_radio]):
            ...
            if not local_pdf:
                missing.append("PDF")
            if not local_extracted:
                missing.append("extracted JSON")
            if not local_mapping:
                missing.append("mapping JSON")
            if not local_radio:
                missing.append("radio groups")
            raise FileNotFoundError(
                f"Required input files not available: {', '.join(missing)}"
            )
        ...
        storage_config = {"type": "local", "path": local_embedded}
        ...
        logger.info("🔍 DEBUG: About to save embedded PDF:")
...
async def handle_fill_operation(
    ...
    try:
        ...
        storage_config = {"type": "local", "path": local_filled}

        # Run Java filler
        await fill_with_java(
        ...
        if config.source_type == "aws" and hasattr(config, "s3_filled_pdf"):
            try:
                ...
                s3_client = S3Client()
                filled_presigned_url = s3_client.generate_presigned_url(
                    saved_path, expires_in=3600
                )
                logger.info(
                    "✅ Generated presigned URL for filled PDF (expires in 1 hour)"
                )
            except Exception as presign_error:
                logger.warning(
                    f"Failed to generate presigned URL for filled PDF: {presign_error}"
                )
        ...
        if notifier and NOTIFICATIONS_AVAILABLE:
            await safe_notify(
                ...
                status=StageStatus.COMPLETED,
                execution_time=duration,
                input_files={"embedded_pdf": local_embedded, "input_json": local_input},
        ...
        if filled_presigned_url:
            ...
            logger.info("Presigned URL included in response")
...
async def handle_run_all_operation(
    ...
    try:
        ...
        map_result["mapping_result"]["radio_groups_path"]
...
async def run_semantic_api_mapper(
    ...
    from pdf_autofillr_mapper.core.config import settings
    from pdf_autofillr_mapper.utils.hash_cache import (
        check_hash_cache,
        copy_cached_files,
    )
    ...
    if pdf_hash:
        try:
            ...
            if cache_result and os.path.exists(cache_registry_path):
                from pdf_autofillr_mapper.handlers.output_handler import (
                    OutputFileHandler,
                )

                cache_output_handler = OutputFileHandler(storage_config)
                cache_dest = cache_output_handler.save_output(
                    cache_registry_path, "cache_registry_json"
                )
                if cache_dest:
                    logger.info(
                        f"📤 Cache registry updated and persisted to: {cache_dest}"
                    )
                else:
            ...
            if cache_result and "reference_files" in cache_result:
                ...
                if (
                    hasattr(storage_config, "cached_mapping_json")
                    and storage_config.cached_mapping_json
                    and hasattr(storage_config, "cached_radio_groups")
                    and storage_config.cached_radio_groups
                ):
                    ...
                    dest_semantic_mapping = cache_result["reference_files"].get(
                        "mapping_json"
                    )
                    dest_radio_groups = cache_result["reference_files"].get(
                        "radio_groups"
                    )
                    ...
                    if semantic_mapping_path and radio_groups_path:
                        logger.info(
                            f"✅ Cached semantic mapping: {semantic_mapping_path}"
                        )
                        ...
                        logger.warning(
                            "❌ Cache hit but files missing, will re-run mapper"
                        )
    ...
    mapper = SemanticMapper(
        llm_provider=(
            mapping_config.get("llm_model", settings.llm_model)
            if mapping_config
            else settings.llm_model
        ),
        confidence_threshold=(
            mapping_config.get("confidence_threshold", 0.7) if mapping_config else 0.7
        ),
        chunking_strategy=(
            mapping_config.get("chunking_strategy", "page")
            if mapping_config
            else "page"
        ),
    )
    ...
    if not local_mapping or not local_radio:
        raise ValueError(
            f"Config missing output paths: local_mapped_json={local_mapping}, local_radio_json={local_radio}"
        )
    ...
    mapper_storage_config = {"output_path": local_mapping, "radio_groups": local_radio}
    ...
    logger.info("✅ Semantic mapper completed")
    ...
    dest_semantic_mapping = output_handler.save_output(
        local_mapping, "semantic_mapping_json"
    )
...
async def run_rag_api_mapper(
    ...
    try:
        ...
        output_handler = OutputFileHandler(storage_config)
        dest_rag_predictions = output_handler.save_output(
            rag_predictions_path, "rag_predictions_json"
        )
        ...
        if (
            ...
            logger.debug(f"RAG shutdown cleanup (non-fatal): {rag_error}")
        else:
...
async def handle_make_embed_file_operation(
    ...
    logger.info("=" * 80)
    logger.info(
        "MAKE EMBED FILE OPERATION - Partial Pipeline (Extract -> Map -> Embed)"
    )
    logger.info("=" * 80)
    ...
    try:
        ...
        input_pdf_s3 = (
            config.s3_input_pdf
            if hasattr(config, "s3_input_pdf") and config.s3_input_pdf
            else config.local_input_pdf
        )
        input_json_s3 = (
            config.s3_global_json
            if hasattr(config, "s3_global_json") and config.s3_global_json
            else (
                config.s3_input_json
                if hasattr(config, "s3_input_json") and config.s3_input_json
                else getattr(config, "local_global_json", None)
                or config.local_input_json
            )
        )
        ...
        if not input_json_s3:
            raise ValueError(
                "config.s3_global_json/s3_input_json or local_input_json not set"
            )
        ...
        if storage_type == "local":
            ...
            file_config = config.get_complete_file_config(
                input_pdf_s3, user_id=user_id, session_id=session_id
            )
        else:
            file_config = None
        ...
        if hasattr(config, "cached_extraction") and config.cached_extraction:
            logger.info(
                "[Cache] ✅ Using cached extraction from entry point (saves ~5s)"
            )
            ...
            if storage_type == "local" and hasattr(config, "local_extracted_json"):
                ...
                temp_extract = tempfile.NamedTemporaryFile(
                    mode="w", suffix="_extracted.json", delete=False
                )
                json.dump(
                    extract_result.get("extracted_data", {}), temp_extract, indent=2
                )
                temp_extract.close()
        ...
        if file_config and storage_type != "local":
            ...
            if extracted_json != expected_path:
                logger.info(
                    f"Moving extracted file to structured output: {expected_path}"
                )
        ...
        if pdf_hash:
            logger.info(
                f"PDF fingerprint hash for pdf_doc_id={pdf_doc_id}: {pdf_hash[:16]}..."
            )
        else:
            logger.warning(
                f"PDF hash not available for caching (pdf_doc_id={pdf_doc_id})"
            )
        ...
        if not cache_registry_path:
            ...
            cache_registry_path = os.path.join(
                settings.data_output_dir, "cache", "hash_registry.json"
            )
            logger.debug(
                f"No cache_registry_path in config, using default: {cache_registry_path}"
            )
        ...
        if pdf_cache_enabled and pdf_hash:
            try:
                ...
                logger.warning(
                    f"Cache check failed: {cache_error}. Proceeding with normal MAP operation."
                )

        # If CACHE HIT: Use cached semantic mapping
        if cache_result:
            logger.info(
                "🎯 CACHE HIT! Using cached semantic mapping (saves ~45s + LLM costs)"
            )
            cache_hit = True
            ...
            try:
                ...
                cached_headers_from_registry = cache_result["reference_files"].get(
                    "headers_with_fields"
                )
                cached_final_fields_from_registry = cache_result["reference_files"].get(
                    "final_form_fields"
                )

                logger.info("🔍 DEBUG: Checking Phase 2 cache:")
                logger.info(
                    f"   cached_headers_from_registry: {cached_headers_from_registry}"
                )
                logger.info(
                    f"   cached_final_fields_from_registry: {cached_final_fields_from_registry}"
                )
                logger.info(
                    f"   config.cached_headers_with_fields: {getattr(config, 'cached_headers_with_fields', None)}"
                )
                logger.info(
                    f"   config.cached_final_form_fields: {getattr(config, 'cached_final_form_fields', None)}"
                )

                headers_with_fields_path = (
                    config.cached_headers_with_fields
                    if hasattr(config, "cached_headers_with_fields")
                    and config.cached_headers_with_fields
                    else cached_headers_from_registry
                )
                final_form_fields_path = (
                    config.cached_final_form_fields
                    if hasattr(config, "cached_final_form_fields")
                    and config.cached_final_form_fields
                    else cached_final_fields_from_registry
                )

                logger.info(
                    f"   Final headers_with_fields_path: {headers_with_fields_path}"
                )
                logger.info(
                    f"   Final final_form_fields_path: {final_form_fields_path}"
                )
                ...
                if use_second_mapper:
                    ...
                    if not headers_with_fields_path or not final_form_fields_path:
                        phase2_needs_caching = (
                            True  # Mark that headers will be freshly created
                        )
                        logger.info("\n" + "-" * 80)
                        logger.info(
                            "MAPPER PHASE 2: RAG API Mapper - Extracting Headers"
                        )
                        logger.info("-" * 80)
                        logger.info(
                            "📝 Headers not cached, extracting headers for RAG API..."
                        )

                        # from pdf_autofillr_mapper.headers import get_form_fields_points
                        from pdf_autofillr_mapper.headers.get_form_fields_points import (
                            get_form_fields_points,
                        )

                        # Get header file paths from config
                        if storage_type == "local" and hasattr(
                            config, "local_headers_with_fields"
                        ):
                            ...
                            headers_output_path = file_config["headers"][
                                "headers_with_fields_path"
                            ]
                            final_fields_output_path = file_config["headers"][
                                "final_form_fields_path"
                            ]
                        else:
                        ...
                        dest_headers_with_fields = output_handler.save_output(
                            headers_with_fields_path, "headers_with_fields_json"
                        )
                        dest_final_form_fields = output_handler.save_output(
                            final_form_fields_path, "final_form_fields_json"
                        )

                        if dest_headers_with_fields:
                            logger.info(
                                f"📤 Uploaded headers_with_fields to: {dest_headers_with_fields}"
                            )
                        if dest_final_form_fields:
                            logger.info(
                                f"📤 Uploaded final_form_fields to: {dest_final_form_fields}"
                            )
                        ...
                        if pdf_hash and phase2_needs_caching:
                            try:
                                ...
                                logger.info(
                                    "💾 Updating cache with Phase 2 results (headers_with_fields + final_form_fields)..."
                                )

                                # Use destination paths for cache (persistent storage)
                                cache_headers = (
                                    dest_headers_with_fields
                                    if dest_headers_with_fields
                                    else headers_with_fields_path
                                )
                                cache_final_fields = (
                                    dest_final_form_fields
                                    if dest_final_form_fields
                                    else final_form_fields_path
                                )

                                # Get cached Phase 1 paths (embedded_pdf, mapping, radio were already cached)
                                cache_mapping = (
                                    dest_semantic_mapping
                                    if dest_semantic_mapping
                                    else semantic_mapping_path
                                )
                                cache_radio = (
                                    dest_radio_groups
                                    if dest_radio_groups
                                ...
                                if os.path.exists(cache_registry_path):
                                    ...
                                    if cache_dest:
                                        logger.info(
                                            f"📤 Cache registry persisted to: {cache_dest}"
                                        )
                                    else:
                                        logger.warning(
                                            "⚠️  Cache registry not uploaded (local storage mode)"
                                        )

                                logger.info(
                                    "✅ Phase 2 cached: headers_with_fields, final_form_fields"
                                )
                            except Exception as cache_error:
                                logger.warning(
                        ...
                        logger.info("\n" + "-" * 80)
                        logger.info(
                            "MAPPER PHASE 2: RAG API Mapper - Using Cached Headers"
                        )
                        logger.info("-" * 80)
                        ...
                        if headers_with_fields_path and final_form_fields_path:
                            ...
                            if not headers_with_fields_path.startswith(
                                "/tmp/processing"
                            ):
                                from pdf_autofillr_mapper.handlers.input_handler import (
                                    InputFileHandler,
                                )
                                ...
                                if storage_type == "local" and hasattr(
                                    ...
                                    processing_headers_path = (
                                        config.local_headers_with_fields
                                    )
                                    processing_final_fields_path = (
                                        config.local_final_form_fields
                                    )
                                elif file_config and "headers" in file_config:
                                    processing_headers_path = file_config["headers"][
                                        "headers_with_fields_path"
                                    ]
                                    processing_final_fields_path = file_config[
                                        "headers"
                                    ]["final_form_fields_path"]
                                else:
                                    raise ValueError(
                                        "Missing header paths configuration"
                                    )

                                # Download files from source to processing using InputFileHandler
                                downloaded_headers = input_handler.download_input(
                                    headers_with_fields_path, processing_headers_path
                                )
                                logger.info(
                                    f"📥 Downloaded cached headers to: {downloaded_headers}"
                                )

                                downloaded_final_fields = input_handler.download_input(
                                    final_form_fields_path, processing_final_fields_path
                                )
                                logger.info(
                                    f"📥 Downloaded cached final_form_fields to: {downloaded_final_fields}"
                                )
                    ...
                    if rag_result["success"]:
                        logger.info(
                            f"✅ RAG predictions: {rag_result['rag_predictions_path']}"
                        )
                        ...
                        dest_llm_predictions = output_handler.save_output(
                            llm_predictions_path, "llm_predictions_json"
                        )
                        logger.info("✅ LLM predictions saved and uploaded")
                        ...
                        logger.info("✅ Combined mapping created")

                        # Upload java mapping and final predictions to source storage
                        output_handler = OutputFileHandler(config)
                        saved_java_mapping = output_handler.save_output(
                            mapping_json, "java_mapping_json"
                        )
                        saved_final_predictions = output_handler.save_output(
                            combined_mapping_path, "final_predictions_json"
                        )
                        logger.info(
                            f"📤 Uploaded Java mapping to: {saved_java_mapping}"
                        )
                        logger.info(
                            f"📤 Uploaded final predictions to: {saved_final_predictions}"
                        )

                        rag_api_failed = False
                    else:
                        logger.warning(
                            f"RAG failed: {rag_result['error']}, using semantic only"
                        )
                    ...
                    logger.info(
                        "🔄 Converting cached semantic mapping to Java format..."
                    )
                    mapping_json = await convert_semantic_to_java_format(
                        semantic_mapping_path=semantic_mapping_path,
                        user_id=user_id,
                ...
                logger.info("✅ Cache processed. MAP stage skipped.")

            except Exception as cache_error:
                logger.error(
                    f"Failed to process cached files: {cache_error}. Running MAP stage."
                )
                cache_hit = False
        ...
        if not cache_hit:
            ...
            if pdf_hash:
                logger.info("📭 CACHE MISS. Running semantic mapper...")
            else:
            ...
            if use_second_mapper:
                ...
                if dest_final_form_fields:
                    logger.info(
                        f"📤 Uploaded final fields to: {dest_final_form_fields}"
                    )
                ...
                if pdf_hash and not cache_hit:
                    try:
                        ...
                        cache_radio = (
                            dest_radio_groups if dest_radio_groups else radio_groups
                        )
                        cache_embedded = (
                            dest_embedded_pdf if dest_embedded_pdf else embedded_pdf
                        )
                ...
                if rag_result["success"]:
                    logger.info(
                        f"✅ RAG mapper completed: {rag_result['rag_predictions_path']}"
                    )
                    rag_predictions_path = rag_result["rag_predictions_path"]
                    rag_result["dest_rag_predictions"]
                    ...
                    logger.info(
                        f"✅ Combined mapping created (Java format): {mapping_json}"
                    )
                    rag_api_failed = False
                else:
        ...
        pipeline_results["map"] = {
            ...
            "combined_mapping_path": (
                combined_mapping_path
                if use_second_mapper and not rag_api_failed
                else None
            ),
            ...
            "rag_failure_reason": (
                rag_failure_reason if use_second_mapper and rag_api_failed else None
            ),
        }
        ...
        if file_config and storage_type != "local":
            ...
            if mapping_json != expected_mapping_path:
                logger.info(
                    f"Moving mapping file to structured output: {expected_mapping_path}"
                )
            ...
            if radio_groups != expected_radio_path:
                logger.info(
                    f"Moving radio groups to structured output: {expected_radio_path}"
                )
        ...
        logger.info("🔍 DEBUG: Extracted destination paths for cache:")
        ...
        if file_config and storage_type != "local":
            ...
            if embedded_pdf != expected_embedded_path:
                logger.info(
                    f"Moving embedded PDF to structured output: {expected_embedded_path}"
                )
        ...
        if pdf_cache_enabled and pdf_hash and not cache_hit:
            try:
                ...
                logger.info(
                    "💾 Saving Phase 1 to cache (embedded_pdf + semantic_mapping + radio_groups)..."
                )
                ...
                logger.info("   Cache will reference persistent paths:")
                ...
                if os.path.exists(cache_registry_path):
                    ...
                    logger.info(
                        f"📤 Persisting cache registry: {cache_registry_path} -> source storage"
                    )
                ...
                logger.info(
                    "✅ Phase 1 cached: embedded_pdf, mapping_json, radio_groups"
                )
            except Exception as cache_error:
                logger.warning(
                    f"Failed to cache Phase 1 results: {cache_error}. Continuing anyway."
                )
        elif cache_hit:
...
async def handle_make_form_fields_data_points(
    ...
    if not input_pdf:
        raise ValueError(
            "config.local_input_pdf not set - AWS handler must download PDF first"
        )
    ...
    try:
        ...
        analysis: dict[str, Any] = {
            ...
            "field_types": {},
            "field_names": [
                f.get("name", "") for f in form_fields if isinstance(f, dict)
            ],
        }
        ...
        analysis_output_path = file_config["extraction"]["extracted_path"].replace(
            ".json", "_analysis.json"
        )
...
async def handle_fill_pdf_operation(
    ...
    embedded_pdf_path = (
        config.s3_embedded_pdf
        if hasattr(config, "s3_embedded_pdf") and config.s3_embedded_pdf
        else config.local_embedded_pdf
    )
    input_json_path = (
        config.s3_input_json
        if hasattr(config, "s3_input_json") and config.s3_input_json
        else config.local_input_json
    )

    if not embedded_pdf_path:
        raise ValueError(
            "config.s3_embedded_pdf or local_embedded_pdf not set - must run embed operation first or set manually"
        )
    if not input_json_path:
        raise ValueError(
            "config.s3_input_json or local_input_json not set - AWS handler must download JSON first"
        )
...
async def handle_check_embed_file_operation(
    config: Any, user_id: Optional[int] = None, session_id: Optional[int] = None
) -> dict[str, Any]:
    """
    ...
    if not embedded_pdf_path:
        raise ValueError(
            "config.local_embedded_pdf not set - must run embed operation first or set manually"
        )
...
async def call_rag_api(
    ...
    rag_files = await create_rag_api_files(
        ...
        or header_file_output_path.replace("header_file.json", "section_file.json"),
        user_id=user_id,
        session_id=session_id,
        pdf_doc_id=pdf_doc_id,
        pdf_hash=pdf_hash,
    ...
    if rag_mode == "inprocess":
        ...
        with open(header_file_local, encoding="utf-8") as f:
            header_data = _json.load(f)

        fields = header_data.get("fields", [])
        pdf_category = header_data.get(
            "pdf_category",
            {
                "category": "unknown",
        ...
        fields = [
            f
            for f in fields
            if (f.get("context") or "").strip()
            or (f.get("section_context") or "").strip()
            or any((h or "").strip() for h in f.get("headers", []))
        ]
    ...
    rag_api_url = getattr(settings, "rag_api_url", "") or os.getenv("RAG_API_URL", "")
    rag_api_key = getattr(settings, "rag_api_key", "") or os.getenv("RAG_API_KEY", "")
    ...
    rag_source_path = (
        result.get("rag_predictions_path")
        or result.get("data", {}).get("rag_predictions")
        or result.get("data", {}).get("s3_paths", {}).get("rag_predictions")
    )
...
async def convert_semantic_to_java_format(
    semantic_mapping_path: str, user_id: int, pdf_doc_id: int, storage_config: Any
) -> str:
    """
    ...
    with open(semantic_mapping_path) as f:
        semantic_data = json.load(f)
    ...
    if not java_mapping_path:
        raise ValueError(
            "Java mapping path not configured. " "Check config.local_java_mapping"
        )
    ...
    logger.info("   ✅ Format: {'field_id': [field_name, '', confidence]}")
    logger.info("   ✅ Middle element is EMPTY STRING (not actual value from input)")

    # Verify format
    with open(java_mapping_path) as f:
        verify_data = json.load(f)
        sample_keys = list(verify_data.keys())[:3]
        logger.info(f"   ✅ Sample keys: {sample_keys}")
        if "user_id" in verify_data or "final_predictions" in verify_data:
            logger.error(
                f"   ❌ ERROR: File has wrong structure! Keys: {list(verify_data.keys())[:10]}"
            )
...
async def save_llm_predictions_to_rag_bucket(
    ...
    if not llm_predictions_path:
        raise ValueError(
            "LLM predictions path not configured. " "Check config.local_llm_predictions"
        )
    ...
    try:
        ...
        rag_dest_path = rag_output_handler.save_output(
            llm_predictions_path, "llm_predictions_json"
        )
        ...
        if rag_dest_path:
            ...
            logger.warning(
                "⚠️  LLM predictions not uploaded to RAG storage (upload returned None)"
            )
    except Exception as upload_error:
        logger.warning(
            f"⚠️  Failed to upload LLM predictions to RAG storage: {upload_error}"
        )
...
async def combine_mappings(
    ...
    with open(rag_predictions_path) as f:
    ...
    for fid in sorted(all_field_ids):
        ...
        llm_field_name = (
            semantic_pred[0] if semantic_pred and semantic_pred[0] else None
        )
        llm_confidence = (
            semantic_pred[2] if semantic_pred and len(semantic_pred) > 2 else 0.0
        )
        ...
        if rag_field_name and llm_field_name:
            ...
            if rag_field_name == llm_field_name:
                ...
                if (rag_confidence or 0.0) >= llm_confidence:
                    selected_name = rag_field_name
                    selected_from = "rag"
                    reason = (
                        "RAG and LLM disagreed, RAG selected due to higher confidence"
                    )
                    ...
                    selected_from = "llm"
                    reason = (
                        "RAG and LLM disagreed, LLM selected due to higher confidence"
                    )
    ...
    logger.info("✅ Final predictions created:")
    ...
    for fid in sorted(all_field_ids):
        ...
        if field_name:
            java_mapping[str(fid)] = [field_name, "", round(float(confidence), 2)]
        else:
    ...
    logger.info(f"📤 Java-compatible mapping saved to: {java_mapping_path}")
    logger.info("   ✅ Format: {'field_id': [field_name, '', confidence]}")
    logger.info("   ✅ This is the EXACT file Java embedder will receive")
    ...
    with open(java_mapping_path) as f:
        ...
        if "user_id" in saved_data or "final_predictions" in saved_data:
            logger.error("   ❌ ERROR: Java mapping file contains wrong structure!")
            logger.error(f"   Keys found: {list(saved_data.keys())[:10]}")
            raise ValueError(
                "Java mapping file has wrong structure - contains detailed predictions instead of simple array format"
            )
        else:
            logger.info(
                "   ✅ Correct format: Keys are field IDs like '1', '2', '3'..."
            )
    ...
    logger.info("✅ Returning both paths:")
