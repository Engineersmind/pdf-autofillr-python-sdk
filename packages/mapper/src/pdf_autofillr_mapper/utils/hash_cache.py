...
async def check_hash_cache(pdf_hash: str, cache_registry_path: str) -> Optional[dict]:
    """
    ...
    logger.info(
        f"Checking hash cache for key: {cache_key[:32]}... at {cache_registry_path}"
    )
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Cache file is corrupted or empty: {e}. Treating as cache miss."
            )
            return None
        ...
        logger.info(
            f"Cache HIT for key: {cache_key[:32]}... (usage_count={entry.get('usage_count', 0)})"
        )
        ...
        try:
            cache_data["last_updated"] = time.strftime(
...
async def save_hash_cache(
    ...
    try:
            ...
            try:
                with open(cache_registry_path) as f:
                    ...
                    if content:
                        ...
                        logger.info(
                            "Cache file exists but is empty. Creating new cache structure."
                        )
                        cache_data = {
                            "version": "1.0",
                ...
                logger.warning(
                    f"Cache file is corrupted ({e}). Creating new cache structure."
                )
                cache_data = {
                    "version": "1.0",
        ...
        if cache_key in entries:
            logger.info(
                f"Cache entry already exists for key: {cache_key[:32]}... Updating with new mappers if provided."
            )
            ...
            if headers_with_fields:
                existing_entry["reference_files"][
                    "headers_with_fields"
                ] = headers_with_fields
            ...
            existing_entry["last_used_at"] = time.strftime(
                "%Y-%m-%dT%H:%M:%SZ", time.gmtime()
            )
        else:
            ...
            entries[cache_key] = {
                ...
                "original_context": {"user_id": user_id, "pdf_doc_id": pdf_doc_id},
            }
        ...
        logger.info(
            f"Successfully saved cache entry. Total entries: {cache_data['total_entries']}"
        )
...
async def copy_cached_files(
    source_files: dict[str, str], target_dir: str
) -> dict[str, str]:
    """
    ...
    try:
        for file_key, source_path in source_files.items():
            ...
            if os.path.abspath(source_path) == os.path.abspath(target_path):
                logger.info(
                    f"Skipped {file_key}: already at target location {target_path}"
                )
...
async def populate_cached_files_to_config(
    pdf_hash: str, cache_registry_path: str, config, download_to_tmp: bool = True
) -> bool:
    """
    ...
    try:
        ...
        tmp_dir = os.path.join(
            tempfile.gettempdir(), f"pdf_cache_{uuid.uuid4().hex[:8]}"
        )
        ...
        if ref_files.get("embedded_pdf"):
            ...
            try:
                downloaded_path = download_from_source(
                    ref_files["embedded_pdf"], tmp_path
                )
                if downloaded_path and os.path.exists(downloaded_path):
                    config.cached_embedded_pdf = downloaded_path
                    logger.info(
                        f"[Cache] ✓ Downloaded embedded_pdf to {downloaded_path}"
                    )
                else:
                    logger.warning("[Cache] Failed to download embedded_pdf")
            except Exception as e:
        ...
        if ref_files.get("mapping_json"):
            ...
            try:
                downloaded_path = download_from_source(
                    ref_files["mapping_json"], tmp_path
                )
                if downloaded_path and os.path.exists(downloaded_path):
                    config.cached_mapping_json = downloaded_path
                    logger.info(
                        f"[Cache] ✓ Downloaded mapping_json to {downloaded_path}"
                    )
                else:
                    logger.warning("[Cache] Failed to download mapping_json")
            except Exception as e:
        ...
        if ref_files.get("radio_groups"):
            ...
            try:
                downloaded_path = download_from_source(
                    ref_files["radio_groups"], tmp_path
                )
                if downloaded_path and os.path.exists(downloaded_path):
                    config.cached_radio_groups = downloaded_path
                    logger.info(
                        f"[Cache] ✓ Downloaded radio_groups to {downloaded_path}"
                    )
                else:
                    logger.warning("[Cache] Failed to download radio_groups")
            except Exception as e:
        ...
        if ref_files.get("headers_with_fields"):
            ...
            try:
                downloaded_path = download_from_source(
                    ref_files["headers_with_fields"], tmp_path
                )
                if downloaded_path and os.path.exists(downloaded_path):
                    config.cached_headers_with_fields = downloaded_path
                    logger.info(
                        f"[Cache] ✓ Downloaded headers_with_fields to {downloaded_path}"
                    )
                else:
                    logger.warning("[Cache] Failed to download headers_with_fields")
            except Exception as e:
        ...
        if ref_files.get("final_form_fields"):
            ...
            try:
                downloaded_path = download_from_source(
                    ref_files["final_form_fields"], tmp_path
                )
                if downloaded_path and os.path.exists(downloaded_path):
                    config.cached_final_form_fields = downloaded_path
                    logger.info(
                        f"[Cache] ✓ Downloaded final_form_fields to {downloaded_path}"
                    )
                else:
                    logger.warning("[Cache] Failed to download final_form_fields")
            except Exception as e:
        ...
        if config.cached_mapping_json and config.cached_radio_groups:
            ...
            logger.warning(
                "[Cache] Failed to download essential cached files (mapping_json, radio_groups)"
            )
            return False
