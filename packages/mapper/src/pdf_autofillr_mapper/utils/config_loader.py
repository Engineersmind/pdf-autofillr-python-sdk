...
class ConfigLoader:
    ...
    def _load_default_config(self):
        ...
        self._config = {
            ...
            "storage": {"use_s3": True, "local_cache_dir": "/tmp/pdf_cache"},
        }
    ...
    def _load_environment_config(self):
        ...
        if os.getenv("OPENAI_API_KEY"):
            self._config["llm"]["providers"]["openai"]["api_key"] = os.getenv(
                "OPENAI_API_KEY"
            )

        if os.getenv("ANTHROPIC_API_KEY"):
            self._config["llm"]["providers"]["claude"]["api_key"] = os.getenv(
                "ANTHROPIC_API_KEY"
            )

        if os.getenv("GOOGLE_API_KEY"):
            self._config["llm"]["providers"]["gemini"]["api_key"] = os.getenv(
                "GOOGLE_API_KEY"
            )
        ...
        if os.getenv("MAX_CONCURRENT_REQUESTS"):
            self._config["processing"]["max_concurrent_requests"] = int(
        ...
        if os.getenv("CONFIDENCE_THRESHOLD"):
            self._config["processing"]["confidence_threshold"] = float(
