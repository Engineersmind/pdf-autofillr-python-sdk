...
class IniConfigLoader:
    ...
    def format_path(self, template_name: str, **kwargs) -> str:
        ...
        try:
            ...
            raise ValueError(
                f"Missing variable {e} for template {template_name}"
            ) from e
    ...
    def get_mapping_config(self) -> dict[str, Any]:
        ...
        if "use_second_mapper" in config:
            config["use_second_mapper"] = config["use_second_mapper"].lower() in (
                "true",
                "yes",
                "1",
            )
    ...
    def get_notification_config(self) -> dict[str, Any]:
        ...
        if "teams_notifications_enabled" in config:
            config["teams_notifications_enabled"] = config[
                "teams_notifications_enabled"
            ].lower() in ("true", "yes", "1")
        if "include_metadata" in config:
            config["include_metadata"] = config["include_metadata"].lower() in (
                "true",
                "yes",
                "1",
            )
