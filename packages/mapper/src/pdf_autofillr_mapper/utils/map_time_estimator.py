...
def _resolve_chunk_size(mapping_config: dict[str, Any]) -> int:
    ...
    strategy = mapping_config.get("chunking_strategy") or chunking_config.get(
        "current_strategy", "page"
    )
...
def _provider_stage_seconds(mapping_config: dict[str, Any]) -> float:
    provider = (
        mapping_config.get("llm_provider") or settings.llm_current_provider or "claude"
    ).lower()
...
def estimate_map_stage_time(
    ...
    if isinstance(pages, list):
        ...
        total_pages = (
            extracted_data.get("total_pages") or extracted_data.get("page_count") or 0
        )
    ...
    return {
        ...
        "totals": {"estimated_seconds": round(estimated_seconds, 2)},
    }
