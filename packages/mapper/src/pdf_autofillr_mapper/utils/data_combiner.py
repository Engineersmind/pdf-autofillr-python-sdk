...
async def combine_user_and_session_data(
    user_id: int, session_id: int, use_profile_info: bool = True
) -> str:
    """
    ...
    session_data_path = (
        f"s3://{bucket}/{user_id}/sessions/{session_id}/final_output_flat.json"
    )
    combined_output_path = (
        f"s3://{bucket}/{user_id}/sessions/{session_id}/final_combined_output_flat.json"
    )
    ...
    try:
        ...
        if use_profile_info:
            ...
            try:
                ...
                logger.warning(
                    f"User profile not found at {user_info_path}, using session data only"
                )
                user_info = {}
