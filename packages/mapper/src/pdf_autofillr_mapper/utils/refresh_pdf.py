...
async def refresh_with_java(
    input_pdf: str, storage_config: Optional[dict[Any, Any]] = None
):
    """
    ...
    possible_jar_paths = [
        ...
        os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "assets", "refresher.jar"
    ...
    if not jar_path:
        logger.error(
            f"[❌] Java refresher JAR not found in any of these locations: {possible_jar_paths}"
        )
        raise FileNotFoundError(
            f"Java refresher JAR not found. Expected locations: {possible_jar_paths}"
        )
    ...
    cmd = ["java", "-jar", jar_path, input_pdf, refreshed_pdf]
    ...
    try:
        ...
        if not os.path.exists(refreshed_pdf):
            raise RuntimeError(
                f"Java process completed but output file not found: {refreshed_pdf}"
            )

        logger.info(
            f"[✅] Java refresh completed successfully. Output: {refreshed_pdf}"
        )
