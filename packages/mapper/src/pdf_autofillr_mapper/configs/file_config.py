...
class FileConfig:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path is None:
            ...
            config_path = str(module_root / "config.ini")

        self.config_path = Path(config_path)
        self.config = configparser.ConfigParser()
        self._sc: Optional[Any] = None
    ...
    def build_file_path(
        ...
        try:
            ...
            raise ValueError(
                f"File naming pattern '{pattern_key}' not found in config.ini"
            ) from e

        # Substitute variables
        filename = pattern.format(
            user_id=user_id, session_id=session_id, pdf_doc_id=pdf_doc_id
        )
    ...
    def get_all_processing_paths(
        self, user_id: int, session_id: str, pdf_doc_id: int
    ) -> dict[str, str]:
        """
        ...
        processing_dir = self.get(
            source_type, "processing_dir", fallback="/tmp/processing"
        )
        ...
        for key in pattern_keys:
            try:
                paths[key] = self.build_file_path(
                    key, user_id, session_id, pdf_doc_id, base_path=processing_dir
                )
            except ValueError:
    ...
    def get_source_input_path(
        self, file_type: str, user_id: int, session_id: str, pdf_doc_id: int
    ) -> str:
        """
        ...
        if file_type == "registry":
            ...
            return self.get(
                source_type, "local_global_json", fallback="/app/data/pdf_registry.json"
            )

        # Get input base path
        input_base = self.get(
            source_type, "input_base_path", fallback="/app/data/input"
        )
        ...
        return self.build_file_path(
            pattern_key, user_id, session_id, pdf_doc_id, base_path=input_base
        )
    ...
    def get_source_output_path(
        ...
        """
        source_type = self.get_source_type()
        output_base = self.get(
            source_type, "output_base_path", fallback="/app/data/output"
        )

        pattern_key = f"output_{file_type}"

        return self.build_file_path(
            pattern_key, user_id, session_id, pdf_doc_id, base_path=output_base
        )
