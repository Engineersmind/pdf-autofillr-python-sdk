...
class AWSStorageConfig(BaseStorageConfig):
    ...
    def get_complete_file_config(
        ...
        # Base paths
        base_key = (
            parsed["key"].rsplit(".", 1)[0] if "." in parsed["key"] else parsed["key"]
        )
        ...
        config = {
            ...
            "dual_mapper": {
                ...
                "header_file_path": (
                    f"s3://{self.rag_bucket_name}/predictions/{user_id}/{session_id}/{base_key.split('/')[-1]}/input_file/header_file.json"
                    if user_id and session_id
                    else None
                ),
                "section_file_path": (
                    f"s3://{self.rag_bucket_name}/predictions/{user_id}/{session_id}/{base_key.split('/')[-1]}/input_file/section_file.json"
                    if user_id and session_id
                    else None
                ),
                "llm_predictions_path": (
                    f"s3://{self.rag_bucket_name}/predictions/{user_id}/{session_id}/{base_key.split('/')[-1]}/predictions/llm_predictions.json"
                    if user_id and session_id
                    else None
                ),
                "rag_predictions_path": (
                    f"s3://{self.rag_bucket_name}/predictions/{user_id}/{session_id}/{base_key.split('/')[-1]}/predictions/rag_predictions.json"
                    if user_id and session_id
                    else None
                ),
                "final_predictions_path": (
                    f"s3://{self.rag_bucket_name}/predictions/{user_id}/{session_id}/{base_key.split('/')[-1]}/predictions/final_predictions.json"
                    if user_id and session_id
                    else None
                ),
