...
async def run_embed_java_stage(
    ...
    mapping_json: str,
    radio_json: str,
    storage_config: Optional[dict[Any, Any]] = None,
):
    """
    ...
    Args:
        ...
        storage_config (dict): Storage configuration for output.
                               If storage_config["path"] is set, that path is used
                               as the output location (so embedded PDF lands inside
                               the session mapper dir, NOT next to the input PDF).
    ...
    # Priority 1: explicit path from storage_config (set by InProcessMapperFiller /
    #             orchestrator so the file lands inside the session mapper dir).
    # Priority 2: legacy behaviour — derive from the input PDF location.
    if storage_config and storage_config.get("path"):
        rebuilt_pdf = storage_config["path"]
        # Ensure the parent directory exists
        os.makedirs(os.path.dirname(rebuilt_pdf), exist_ok=True)
        logger.info(f"[📁] Using configured output path: {rebuilt_pdf}")
    elif "/tmp/" in original_pdf:
        ...
        # Legacy fallback — output sits next to the input PDF
    ...
    possible_jar_paths = [
        # os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "java_utils", "form_field_rebuilder", "target", "form_field_rebuilder-1.0.0-jar-with-dependencies.jar"),  # Bundled in wheel
        os.path.join(
            os.path.dirname(os.path.dirname(__file__)),
            "java_utils",
            "form_field_rebuilder",
            "target",
            "form_field_rebuilder-1.0.0-jar-with-dependencies.jar",
        ),  # Bundled in wheel
        ...
        os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "assets", "rebuilder.jar"
    ...
    if not jar_path:
        logger.error(
            f"[❌] Java rebuilder JAR not found in any of these locations: {possible_jar_paths}"
        )
        raise FileNotFoundError(
            f"Java rebuilder JAR not found. Expected locations: {possible_jar_paths}"
        )
    ...
    try:
        ...
        if not os.path.exists(rebuilt_pdf):
            raise RuntimeError(
                f"Java process completed but output file not found: {rebuilt_pdf}"
            )

        logger.info(
            f"[✅] Java embedding completed successfully. Output: {rebuilt_pdf}"
        )
        return rebuilt_pdf
