...
class Settings(BaseSettings):
    ...
    pdf_cache_enabled: bool = (
        True  # Enable/disable hash cache for MAP stage optimization
    )
    pdf_cache_bucket: str = (
        ""  # S3 bucket for cache storage (defaults to storage_s3_bucket if empty)
    )
    ...
    cache_registry_path: str = get_ini_value(
        get_source_type(), "cache_registry_path", ""
    )
    ...
    headers_openai_model_id: str = get_ini_value(
        "headers", "headers_openai_model_id", "gpt-4o"
    )
    headers_claude_model_id: str = get_ini_value(
        "headers", "headers_claude_model_id", "claude-3-5-sonnet-20241022"
    )
    ...
    return {
        "method": {
            ...
            "methods": [
                ...
                {
                    "name": "embed",
                    "embedder": {"current_provider": settings.mapper_embed_provider},
                },
            ],
        },
        "chunking": {
...
def get_file_paths(pdf_filename: str) -> dict[str, str]:
    ...
    return {
        ...
        "extracted_path": os.path.join(
            settings.temp_data_dir, f"{base_name}_extracted.json"
        ),
        "input_json_path": os.path.join(
            settings.data_input_dir, f"{base_name}_input_keys.json"
        ),
        "input_key_variants_path": os.path.join(
            settings.data_input_dir, f"{base_name}_key_variants.json"
        ),
        "field_name_variants_path": os.path.join(
            settings.data_input_dir, f"{base_name}_field_variants.json"
        ),
        # Output paths
        "output_mappings_path": os.path.join(
            settings.data_output_dir, f"{base_name}_mappings.json"
        ),
        "radio_groups_path": os.path.join(
            settings.data_output_dir, f"{base_name}_radio_groups.json"
        ),
        "filled_pdf_path": os.path.join(
            settings.data_output_dir, f"{base_name}_filled.pdf"
        ),
    }
...
def get_processing_output_config(
    ...
    if extracted_json_path.startswith("s3://"):
        ...
        return {
            "radio_groups_path": os.path.join(
                folder, f"{base_name}{session_suffix}_radio_groups.json"
            ),
            "mapping_path": os.path.join(
                folder, f"{base_name}{session_suffix}_mapping.json"
            ),
            "filled_pdf_path": os.path.join(
                folder, f"{base_name}{session_suffix}_filled.pdf"
            ),
            "storage_type": "local",
            "folder": folder,
            "base_name": base_name,
    ...
    if extracted_json_path.startswith("s3://"):
        ...
        return {
            "headers_with_fields_path": os.path.join(
                folder, f"{base_name}{session_suffix}_headers_with_fields.json"
            ),
            "final_form_fields_path": os.path.join(
                folder, f"{base_name}{session_suffix}_final_form_fields.json"
            ),
            "storage_type": "local",
            "folder": folder,
            "base_name": base_name,
...
def get_lambda_storage_config(
    file_path: str, output_type: str = "mapping"
) -> dict[str, Any]:
    """
    ...
    if output_type == "extraction":
        ...
        return {"type": config["storage_type"], "path": config["extracted_path"]}
...
def get_complete_file_config(
    ...
    return storage.get_complete_file_config(
        input_file_path, user_id=user_id, session_id=session_id
    )
