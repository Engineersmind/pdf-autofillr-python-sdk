...
class APIClient:
    ...
    def __init__(
        ...
        self._own_auth_client = (
            auth_client is None
        )  # Track if we created the auth client
    ...
    async def get_document_s3_url(self, doc_id: int, is_presigned: bool = False) -> str:
        ...
        try:
            ...
            async with self._session.get(
                ...
                headers={"accept": "*/*", "Authorization": f"Bearer {self._token}"},
            ) as response:
                ...
                if s3_url and ".s3." in s3_url:
                    ...
                    logger.warning(
                        f"Could not parse bucket from s3_url, using default: {bucket_name}"
                    )
