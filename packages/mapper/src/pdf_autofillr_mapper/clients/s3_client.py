...
class S3Client:
    ...
    def __init__(
        ...
        try:
            if profile_name:
                ...
                logger.info(
                    f"S3 client initialized with profile '{profile_name}' successfully"
                )
            else:
    ...
    def parse_s3_path(self, s3_path: str) -> tuple[str, str]:
        ...
        if "/" not in path_without_prefix:
            raise ValueError(
                f"Invalid S3 path format: {s3_path}. Must include bucket and key"
            )
        ...
        if not bucket or not key:
            raise ValueError(
                f"Invalid S3 path format: {s3_path}. Both bucket and key must be specified"
            )
    ...
    def read_pdf_from_s3(self, s3_path: str) -> bytes:
        ...
        try:
            ...
            if error_code == "NoSuchKey":
                ...
                raise FileNotFoundError(f"PDF file not found in S3: {s3_path}") from e
    ...
    def load_json_from_s3(self, s3_path: str) -> dict:
        ...
        try:
            ...
            if error_code == "NoSuchKey":
                ...
                raise FileNotFoundError(f"JSON file not found in S3: {s3_path}") from e
            elif error_code == "NoSuchBucket":
                logger.error(f"S3 bucket not found: {bucket}")
                raise FileNotFoundError(f"S3 bucket not found: {bucket}") from e
            else:
            ...
            raise ValueError(
                f"Invalid JSON content in S3 file {s3_path}: {str(e)}"
            ) from e
        except Exception as e:
    ...
    def download_file_from_s3(
        self, s3_path: str, local_path: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            logger.info(
                f"Downloading file from S3: bucket={bucket}, key={key} -> {local_path}"
            )
            ...
            try:
                ...
                if head_error.response["Error"]["Code"] == "404":
                    ...
                    raise FileNotFoundError(
                        f"S3 file does not exist: {s3_path}"
                    ) from head_error
            ...
            if not os.path.exists(local_path):
                raise RuntimeError(
                    f"File download completed but file not found at: {local_path}"
                )

            file_size = os.path.getsize(local_path)
            logger.info(
                f"Successfully downloaded file from S3: {local_path} ({file_size} bytes)"
            )
            return local_path
            ...
            if error_code in ["NoSuchKey", "404"]:
                logger.error(f"File not found in S3: {s3_path}")
                raise FileNotFoundError(f"S3 file does not exist: {s3_path}") from e
            elif error_code == "NoSuchBucket":
                logger.error(f"S3 bucket not found: {bucket}")
                raise FileNotFoundError(f"S3 bucket does not exist: {bucket}") from e
            else:
    ...
    def upload_file_to_s3(self, local_path: str, s3_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Uploading file to S3: {local_path} -> bucket={bucket}, key={key}"
            )
    ...
    def list_bucket_objects(self, bucket_name: str, prefix: str = "") -> list:
        ...
        try:
            response = self.s3_client.list_objects_v2(
                Bucket=bucket_name, Prefix=prefix, MaxKeys=20  # Limit for debugging
            )
    ...
    def generate_presigned_url(
        self, s3_path: str, expires_in: int = 3600, inline: bool = True
    ) -> str:
        """
        ...
        if not s3_path.startswith("s3://"):
            raise ValueError(
                f"Cannot generate presigned URL for non-S3 path: {s3_path}"
            )
    ...
    def copy_object(self, source_s3_path: str, target_s3_path: str) -> bool:
        ...
        try:
            ...
            if error_code == "NoSuchKey":
                ...
                raise FileNotFoundError(
                    f"Source object not found: {source_s3_path}"
                ) from e
            elif error_code == "NoSuchBucket":
                logger.error("Bucket not found in copy operation")
                raise FileNotFoundError("Bucket not found during copy operation") from e
            else:
