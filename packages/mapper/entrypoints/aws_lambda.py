...
async def route_operation(event: dict, operation: str, notifier):
    ...
    if operation == "extract":
        ...
        if session_id is not None:
            ...
            if not user_id:
                raise ValueError(
                    "Missing required parameter: user_id (required with session_id)"
                )
            if not pdf_doc_id:
                raise ValueError("Missing required parameter: pdf_doc_id")

            logger.info(
                f"Session-based workflow - user_id: {user_id}, session_id: {session_id}, pdf_doc_id: {pdf_doc_id}"
            )
            ...
            logger.info(
                f"Doc-based workflow - PDF doc_id: {pdf_doc_id}, JSON doc_id: {input_json_doc_id}"
            )
            ...
            async with APIClient() as api_client:
                ...
                input_json = await api_client.get_document_s3_url(
                    doc_id=input_json_doc_id
                )
                ...
                if not input_json.lower().endswith(".json"):
                    raise ValueError(
                        f"input_json_doc_id must be a JSON file, got: {input_json}"
                    )
    ...
    # Other operations - now fully implemented
        ...
        if user_id is None:
            raise ValueError(
                "Missing required parameter: user_id for make_embed_file operation"
            )
        if pdf_doc_id is None:
            raise ValueError(
                "Missing required parameter: pdf_doc_id for make_embed_file operation"
            )
        ...
        if config.global_input_json_s3_uri:
            local_global_json = "/tmp/global_input_keys.json"
            try:
        ...
        from pdf_autofillr_mapper.utils.hash_cache import (
            populate_cached_files_to_config,
        )
        ...
        if settings.pdf_cache_enabled:
            try:
                ...
                if s3_cache_registry_path and s3_cache_registry_path.startswith(
                    "s3://"
                ):
                    ...
                    try:
                        download_from_source(
                            s3_cache_registry_path, local_cache_registry
                        )
                        logger.info(
                            "[Cache] ✓ Downloaded hash_registry.json from S3 to /tmp"
                        )
                    except Exception as e:
                        logger.info(
                            f"[Cache] hash_registry.json doesn't exist in S3 yet (first run): {e}"
                        )
                    ...
                    from pdf_autofillr_mapper.extractors.detailed_fitz import (
                        DetailedFitzExtractor,
                    )
                    ...
                    if pdf_hash:
                        ...
                        if cache_hit:
                            logger.info(
                                "[Cache] ✅ Config populated with cached files from /tmp"
                            )
                        else:
                            logger.info("[Cache] Cache miss - will run full pipeline")
                    else:
                        logger.warning(
                            "[Cache] Could not generate PDF hash from extraction"
                        )
                else:
                    logger.info(
                        "[Cache] cache_registry_path is not an S3 path, skipping cache download"
                    )
        ...
        if (
            settings.pdf_cache_enabled
            and local_cache_registry
            and s3_cache_registry_path
        ):
            try:
                if os.path.exists(local_cache_registry):
                    ...
                    logger.warning(
                        f"[Cache] Local cache registry not found at {local_cache_registry}"
                    )
            except Exception as upload_err:
                logger.error(
                    f"[Cache] Failed to upload hash_registry.json to S3: {upload_err}"
                )
        ...
        if user_id is None:
            raise ValueError(
                "Missing required parameter: user_id for make_form_fields_data_points operation"
            )
        if pdf_doc_id is None:
            raise ValueError(
                "Missing required parameter: pdf_doc_id for make_form_fields_data_points operation"
            )
        ...
        if user_id is None:
            raise ValueError(
                "Missing required parameter: user_id for fill_pdf operation"
            )
        if pdf_doc_id is None:
            raise ValueError(
                "Missing required parameter: pdf_doc_id for fill_pdf operation"
            )
        if session_id is None:
            raise ValueError(
                "Missing required parameter: session_id for fill_pdf operation"
            )
        ...
        async with APIClient() as api_client:
            ...
            embedded_pdf_s3_url = await api_client.get_document_s3_url(
                doc_id=pdf_doc_id
            )
            ...
            if not embedded_pdf_s3_url.lower().endswith(".pdf"):
                raise ValueError(
                    f"embedded_pdf_path must be a PDF file, got: {embedded_pdf_s3_url}"
                )
        ...
        if user_id is None:
            raise ValueError(
                "Missing required parameter: user_id for check_embed_file operation"
            )
        if pdf_doc_id is None:
            raise ValueError(
                "Missing required parameter: pdf_doc_id for check_embed_file operation"
            )
        ...
        return await handle_check_embed_file_operation(
            config=config, user_id=user_id, session_id=event.get("session_id")
        )
