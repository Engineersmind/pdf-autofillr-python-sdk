...
class PipelineNotifier:
    ...
    def start_pipeline(
        ...
        self._pipeline_id = (
            pipeline_id or f"pipeline_{int(time.time())}_{uuid.uuid4().hex[:8]}"
        )
    ...
    async def notify_stage_completion(
        ...
        if self.http_client:
            try:
                response: Any = await self.http_client.send_notification(
                    payload=payload, operation_name=f"{stage.value}_stage"
                )

                if response.success:
                    http_success = True
                    logger.info(
                        f"✅ HTTP Stage notification sent: {stage.value} -> {status.value}"
                    )
                else:
                    logger.warning(
                        f"❌ HTTP Stage notification failed: {stage.value} -> {status.value}"
                    )
        ...
        if self.teams_client:
            try:
                ...
                response = await self.teams_client.send_message(
                    payload=teams_payload, operation_name=f"{stage.value}_stage"
                )

                if response.success:
                    teams_success = True
                    logger.info(
                        f"✅ Teams Stage notification sent: {stage.value} -> {status.value}"
                    )
                else:
                    logger.warning(
                        f"❌ Teams Stage notification failed: {stage.value} -> {status.value}"
                    )
    ...
    async def notify_pipeline_completion(
        ...
        if self.http_client:
            try:
                response: Any = await self.http_client.send_notification(
                    payload=payload, operation_name="pipeline_completion"
                )

                if response.success:
                    http_success = True
                    logger.info(
                        f"🎉 HTTP Pipeline completion notification sent: {status.value}"
                    )
                else:
                    logger.error(
                        f"💥 HTTP Pipeline completion notification failed: {status.value}"
                    )
        ...
        if self.teams_client:
            try:
                ...
                response = await self.teams_client.send_message(
                    payload=teams_payload, operation_name="pipeline_completion"
                )

                if response.success:
                    teams_success = True
                    logger.info(
                        f"🎉 Teams Pipeline completion notification sent: {status.value}"
                    )
                else:
                    logger.error(
                        f"💥 Teams Pipeline completion notification failed: {status.value}"
                    )
    ...
    def _build_teams_stage_card(
        ...
        if user_input_details:
            if user_input_details.get("user_id"):
                facts.append(
                    {"title": "User", "value": str(user_input_details["user_id"])}
                )
            if user_input_details.get("session_id"):
                facts.append(
                    {"title": "Session", "value": str(user_input_details["session_id"])}
                )
    ...
    def _build_teams_pipeline_card(
        ...
        if stage_breakdown:
            ...
            sections.append({"activityTitle": "⏱️ Stage Timing", "facts": timing_facts})
