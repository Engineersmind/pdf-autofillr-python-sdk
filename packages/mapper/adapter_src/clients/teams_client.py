...
class TeamsClient:
    ...
    async def send_message(
        self, payload: dict[str, Any], operation_name: str = "teams_message"
    ) -> TeamsResponse:
        """
    ...
    async def _send_request(
        ...
        async with session.post(
            ...
            if response.status in [200, 202]:
                logger.info(
                    f"✅ [Teams {operation_name}] Sent successfully ({response_time:.0f}ms)"
                )
...
def create_teams_client_from_config() -> Optional[TeamsClient]:
    ...
    try:
        ...
        if not webhook_url:
            logger.info(
                "Teams webhook URL not configured - Teams notifications disabled"
            )
            return None
        ...
        return TeamsClient(
            webhook_url=webhook_url, timeout=timeout, max_retries=max_retries
        )
