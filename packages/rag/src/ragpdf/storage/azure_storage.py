...
class AzureStorage(StorageBackend):
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "AzureStorage requires azure-storage-blob. Install with: pip install ragpdf-sdk[azure]"
            ) from e
        ...
        if conn:
            ...
            raise ValueError(
                "AzureStorage requires RAGPDF_AZURE_CONN_STR or RAGPDF_AZURE_ACCOUNT"
            )
        ...
        try:
            ...
            pass  # already exists
        logger.info(
            f"AzureStorage initialized: container={self._container}, prefix={self._prefix}"
        )
