...
class ChromaStore(VectorStoreBackend):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("pip install ragpdf-sdk[chroma]") from e
        from ragpdf.config.settings import RAGPDF_CHROMA_COLLECTION, RAGPDF_CHROMA_PATH
        ...
        logger.info(
            f"ChromaStore initialized: {self._col.name} ({self._col.count()} vectors)"
        )
    ...
    def find_similar(
        ...
        top_k_r = [
            {
                "field_name": metas[i].get("field_name", ""),
                "confidence": sims[i],
                "vector_id": ids[i],
            }
        ...
        return {
            "matched": False,
            "confidence": bc,
            "best_candidate": best_meta.get("field_name", ""),
            "top_k": top_k_r,
            "similarity_margin": float(margin),
        }
