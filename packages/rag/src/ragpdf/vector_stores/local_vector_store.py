...
class LocalVectorStore(VectorStoreBackend):
    ...
    def find_similar(self, query_embedding: list,
        ...
        if np.linalg.norm(query_embedding) < 1e-9:
            ...
            return {
                ...
                "top_k": [], "similarity_margin": 0.0, "best_candidate": None,
            }

        # Guard against vectors with missing embeddings (source-only, not yet embedded)
        embeddable = [v for v in vectors if v.get("embedding") and len(v["embedding"]) > 0
        ...
        margin    = (
            ...
            if len(top_k_results) >= 2 else 0.0
        )
    ...
    def update_confidence(self, vector_id: str, is_positive: bool,
        ...
        for vector in self.data["vectors"]:
            ...
            vector["stability_score"] = round(
                vector.get("positive_count", 0) / total, 4
            ) if total else 1.0
