...
class VectorStoreFactory:
    ...
    def create() -> VectorStoreBackend:
        from ragpdf.config.settings import (
            RAGPDF_VECTOR_STORE, RAGPDF_DATA_PATH,
            ...
            PINECONE_API_KEY, RAGPDF_PINECONE_INDEX, RAGPDF_PINECONE_NAMESPACE,
            RAGPDF_CHROMA_PATH, RAGPDF_CHROMA_COLLECTION,
            RAGPDF_WEAVIATE_URL, RAGPDF_WEAVIATE_API_KEY, RAGPDF_WEAVIATE_CLASS,
        )
        ...
        if RAGPDF_VECTOR_STORE == "azure":
            ...
            return AzureVectorStore(account=RAGPDF_AZURE_ACCOUNT, container=RAGPDF_AZURE_CONTAINER, conn_str=RAGPDF_AZURE_CONN_STR)
        ...
        if RAGPDF_VECTOR_STORE == "pinecone":
            ...
            return PineconeStore(api_key=PINECONE_API_KEY, index_name=RAGPDF_PINECONE_INDEX, namespace=RAGPDF_PINECONE_NAMESPACE)

        if RAGPDF_VECTOR_STORE == "chroma":
            from ragpdf.vector_stores.chroma_store import ChromaStore
            return ChromaStore(path=RAGPDF_CHROMA_PATH, collection=RAGPDF_CHROMA_COLLECTION)

        if RAGPDF_VECTOR_STORE == "weaviate":
            from ragpdf.vector_stores.weaviate_store import WeaviateStore
            return WeaviateStore(url=RAGPDF_WEAVIATE_URL, api_key=RAGPDF_WEAVIATE_API_KEY, class_name=RAGPDF_WEAVIATE_CLASS)
