...
class WeaviateStore(VectorStoreBackend):
    ...
    def __init__(
        self, url: str = "", api_key: str = "", class_name: str = "RagpdfVector"
    ):
        try:
            import weaviate
        except ImportError as e:
            raise ImportError("pip install ragpdf-sdk[weaviate]") from e
        ...
        auth = (
            weaviate.auth.AuthApiKey(api_key or RAGPDF_WEAVIATE_API_KEY)
            if (api_key or RAGPDF_WEAVIATE_API_KEY)
            else None
        )
        self._client = weaviate.connect_to_custom(
            http_host=url or RAGPDF_WEAVIATE_URL, auth_credentials=auth
        )
    ...
    def find_similar(
        ...
        top_k_r = [
            {
                "field_name": o.properties.get("field_name", ""),
                "confidence": o.metadata.certainty or 0.0,
                "vector_id": str(o.uuid),
            }
            for o in objs
        ]
        bc = top_k_r[0]["confidence"]
        best = objs[0]
        margin = (
        ...
        if bc >= threshold:
            ...
            return {
                "matched": True,
                "vector_id": str(best.uuid),
                "field_name": p.get("field_name", ""),
                "confidence": bc,
                "vector_confidence": float(p.get("current_confidence", 0.75)),
        ...
        return {
            "matched": False,
            "confidence": bc,
            "best_candidate": top_k_r[0]["field_name"],
            "top_k": top_k_r,
            "similarity_margin": float(margin),
        }
    ...
    def update_confidence(self, vector_id, is_positive, error_info=None):
        ...
        if not obj:
            return None
        p = obj.properties
        curr = float(p.get("current_confidence", 0.75))
        if is_positive:
            new_c = min(curr * CONFIDENCE_GROWTH_RATE, MAX_CONFIDENCE)
            new_pos = int(p.get("positive_count", 0)) + 1
            new_neg = int(p.get("negative_count", 0))
        else:
            new_c = max(curr * CONFIDENCE_DECAY_RATE, MIN_CONFIDENCE)
            new_pos = int(p.get("positive_count", 0))
            new_neg = int(p.get("negative_count", 0)) + 1
        t = new_pos + new_neg
        col.data.update(
            uuid=uuid.UUID(vector_id),
            properties={
                "current_confidence": round(new_c, 6),
