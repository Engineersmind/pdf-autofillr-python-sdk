...
class AzureVectorStore(VectorStoreBackend):
    def __init__(self, account: str = "", container: str = "", conn_str: str = "", prefix: str = ""):
        ...
        self._storage = AzureStorage(account=account, container=container, conn_str=conn_str, prefix=prefix)
    ...
    def add_vector(self, field_name, context, section_context, headers, embedding, **metadata):
        if not any(x != 0.0 for x in embedding):
            raise ValueError(f"Rejected zero embedding for field '{field_name}' — NoOp embedder must not be used in production")
            ...
            if is_positive:
                ...
                if error_info:
                    v.setdefault("error_history", []).append({"timestamp": now, "pdf_hash": error_info.get("pdf_hash"),
                        "error_type": error_info.get("error_type"), "user_feedback": error_info.get("user_feedback"),
