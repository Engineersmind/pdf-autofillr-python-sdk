...
class S3VectorStore(VectorStoreBackend):
    def __init__(self, bucket: str, region: str = "us-east-1", prefix: str = ""):
        try:
            ...
            raise ImportError("pip install ragpdf-sdk[s3]") from e
    ...
    def find_similar(
        ...
        if bc >= threshold:
            bv["usage_count"] = bv.get("usage_count", 0) + 1
            bv["last_used"] = _now()
            return {
                "matched": True,
                "vector_id": bv["vector_id"],
                "field_name": bv["field_name"],
                "confidence": bc,
                "vector_confidence": bv.get("confidence_history", [0.75])[-1],
                "positive_count": bv.get("positive_count", 0),
                "negative_count": bv.get("negative_count", 0),
                "usage_count": bv.get("usage_count", 0),
                "stability_score": bv.get("stability_score", 1.0),
                "top_k": top_k_r,
    ...
    def add_vector(
        ...
        vid = generate_vector_id(self.data["vectors"])
        now = _now()
