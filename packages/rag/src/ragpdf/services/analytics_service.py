...
class AnalyticsService:
    ...
    def get_metrics(self, body: dict) -> dict:
        ...
        fn = dispatch.get(mt) if mt is not None else None
        if not fn:
    ...
    def _pdf_metrics(self, body):
        ...
        return {
            ...
            "initial_metrics": snap,
            "current_metrics": updated or snap,
    ...
    def _category_metrics(self, body):
        ...
        ts = self.storage.load_json(
            f"metrics/time_series/category/{cat}/time_series.json"
        )
        if not ts:
            return {
        ...
        return {
            ...
            "time_range": (
                {
                    "start": ts["metadata"]["first_entry"],
                    "end": ts["metadata"]["last_entry"],
                }
                if ts.get("metadata")
                else None
            ),
            "time_series": entries,
            "aggregated": _agg(entries),
        }

    def _subcategory_metrics(self, body):
        cat, sub = body.get("category"), body.get("subcategory")
        if not all([cat, sub]):
            raise ValueError("category and subcategory required")
        ts = self.storage.load_json(
            f"metrics/time_series/subcategory/{cat}/{sub}/time_series.json"
        )
        return {
    ...
    def _doctype_metrics(self, body):
        ...
        if not all([cat, sub, doc]):
            raise ValueError("category, subcategory, doctype required")
        ts = self.storage.load_json(
            f"metrics/time_series/doctype/{cat}/{sub}/{doc}/time_series.json"
        )
        return {
            "metric_type": "doctype",
            "category": cat,
    ...
    def _global_metrics(self, body):
        ...
        def avg(k):
            vs = [e["metrics"][k] for e in entries if e["metrics"].get(k) is not None]
            return round(sum(vs) / len(vs), 4) if vs else 0.0

        def tot(k):
            return sum(e["metrics"].get(k, 0) for e in entries)

        def win_high(a, b):
            return "llm" if a > b else ("rag" if b > a else "tie")
        ...
        overall = {
            ...
            "total_fields_predicted": {
                "llm": tot("predicted_llm"),
                "rag": tot("predicted_rag"),
                "ensemble": tot("predicted_ensemble"),
            },
            "total_errors": {
                "llm": tot("errors_llm"),
                "rag": tot("errors_rag"),
                "ensemble": tot("errors_ensemble"),
            },
            "avg_accuracy": {
                "llm": avg("accuracy_llm"),
                "rag": avg("accuracy_rag"),
                "ensemble": avg("accuracy_ensemble"),
            },
            "avg_coverage": {
                "llm": avg("coverage_llm"),
                "rag": avg("coverage_rag"),
                "ensemble": avg("coverage_ensemble"),
            },
            "avg_confidence": {
                "llm": avg("avg_conf_llm"),
                "rag": avg("avg_conf_rag"),
                "ensemble": avg("avg_conf_ensemble"),
            },
            "agreement_rate": avg("agreement_rate"),
            "conflict_rate": avg("conflict_rate"),
            "recovery": {
                "rag_recovers_llm_misses": avg("rag_recovery"),
                "llm_recovers_rag_misses": avg("llm_recovery"),
            },
        }
        llm_rag = {
            "accuracy": {
                "llm": avg("accuracy_llm"),
                "rag": avg("accuracy_rag"),
                "winner": win_high(avg("accuracy_llm"), avg("accuracy_rag")),
            },
            "coverage": {
                "llm": avg("coverage_llm"),
                "rag": avg("coverage_rag"),
                "winner": win_high(avg("coverage_llm"), avg("coverage_rag")),
            },
            "confidence": {
                "llm": avg("avg_conf_llm"),
                "rag": avg("avg_conf_rag"),
                "winner": win_high(avg("avg_conf_llm"), avg("avg_conf_rag")),
            },
            "errors": {
                "llm": tot("errors_llm"),
                "rag": tot("errors_rag"),
                "winner": win_low(tot("errors_llm"), tot("errors_rag")),
            },
            "agreement_rate": avg("agreement_rate"),
            "conflict_rate": avg("conflict_rate"),
        }
    ...
    def _compare_pdfs(self, body):
        pdfs = body.get("pdfs", [])
        if not pdfs:
            raise ValueError("pdfs array required")
        comparison = []
        for pdf in pdfs:
            uid, sid, pid = pdf.get("user_id"), pdf.get("session_id"), pdf.get("pdf_id")
            if not all([uid, sid, pid]):
                continue
            snap = self.storage.load_json(
                f"predictions/{uid}/{sid}/{pid}/analysis/metrics_snapshot.json"
            )
            upd = self.storage.load_json(
                f"predictions/{uid}/{sid}/{pid}/errors/metrics_snapshot_updated.json"
            )
            cur = upd or snap
            if cur:
                comparison.append(
    ...
    def _pdf_hash_metrics(self, body):
        ...
        if not ph:
            raise ValueError("pdf_hash required")
            ...
            subs.append(
                {
                    **s,
                    "metrics": (upd or snap),
                    "errors": ea.get("errors", []) if ea else [],
                    "total_errors": len(ea.get("errors", [])) if ea else 0,
                }
            )
        return {
            "metric_type": "pdf_hash",
            ...
            "total_submissions": entry.get("total_submissions", 0),
            "submissions": subs,
        }
    ...
    def _system_info(self):
        ...
        for e in mapping.values():
            for s in e.get("submissions", []):
                ...
                if s.get("session_id"):
                    all_sessions.add(s["session_id"])
        cats, subs, docs = {}, {}, {}
        for _ph, e in mapping.items():
            cat, sub, doc = (
                e.get("category", "?"),
                e.get("sub_category", "?"),
                e.get("document_type", "?"),
            ...
            cats.setdefault(cat, {"pdf_count": 0, "submission_count": 0})
            cats[cat]["pdf_count"] += cnt
            cats[cat]["submission_count"] += tot
            sk = f"{cat}/{sub}"
            subs.setdefault(
                sk,
                {
            ...
            subs[sk]["pdf_count"] += cnt
            subs[sk]["submission_count"] += tot
            dk = f"{cat}/{sub}/{doc}"
            docs.setdefault(
                dk,
                {
            ...
            docs[dk]["pdf_count"] += cnt
            docs[dk]["submission_count"] += tot
        return {
            "metric_type": "system_info",
            "generated_at": datetime.utcnow().isoformat() + "Z",
            "summary": {
                "total_pdf_hashes": len(mapping),
                "total_submissions": sum(
                    e.get("total_submissions", 0) for e in mapping.values()
                ),
                "total_unique_users": len(all_users),
                "total_unique_sessions": len(all_sessions),
                "total_categories": len(cats),
                "total_subcategories": len(subs),
                "total_document_types": len(docs),
            ...
            "subcategories": [
                v
                for v in sorted(
                    subs.values(), key=lambda x: (x["category"], x["sub_category"])
                )
            ],
            "document_types": [
                v
            ...
            "vector_db": {
                ...
                "sources": {
                    "rag": sum(
                        1 for v in vectors if v.get("prediction_source") == "rag"
                    ),
                    "llm": sum(
                        1 for v in vectors if v.get("prediction_source") == "llm"
                    ),
                    "manual": sum(
                        1 for v in vectors if v.get("prediction_source") == "manual"
                    ),
                },
            },
        }
    ...
    def get_error_analytics(self, body: dict) -> dict:
        ...
        for ph, he in mapping.items():
            ...
            for s in he.get("submissions", []):
                uid, sid, pid = s.get("user_id"), s.get("session_id"), s.get("pdf_id")
                if not (uid and sid and pid):
                    continue
                ea = self.storage.load_json(
                    f"predictions/{uid}/{sid}/{pid}/errors/error_analysis.json"
                )
                if not ea:
                    continue
        ...
        for e in all_errors:
            ...
            sk = f"{e['category']}/{e['sub_category']}"
            by_sub[sk] = by_sub.get(sk, 0) + 1
            dk = f"{e['category']}/{e['sub_category']}/{e['document_type']}"
            by_doc[dk] = by_doc.get(dk, 0) + 1
            day = (e.get("timestamp") or "")[:10]
            if day:
                by_date[day] = by_date.get(day, 0) + 1
            et = e.get("error_type", "unknown")
            by_et[et] = by_et.get(et, 0) + 1
            ct = e.get("case_type", "unknown")
        ...
        return {
            ...
            "breakdown": {
                "by_category": [
                    {"category": k, "error_count": v} for k, v in sorted(by_cat.items())
                ],
                "by_subcategory": [
                    {"path": k, "error_count": v} for k, v in sorted(by_sub.items())
                ],
                "by_doctype": [
                    {"path": k, "error_count": v} for k, v in sorted(by_doc.items())
                ],
                "by_date": [
                    {"date": k, "error_count": v} for k, v in sorted(by_date.items())
                ],
                "by_error_type": [
                ...
                "by_case_type": [
                    {"case_type": k, "error_count": v} for k, v in sorted(by_ct.items())
                ],
            },
            "errors": all_errors,
        }
...
def _agg(entries):
    ...
    return {
        ...
        "avg_coverage": round(sum(covs) / len(covs), 4),
