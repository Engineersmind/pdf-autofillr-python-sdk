...
class MetricsService:
    ...
    def calculate_metrics(
        ...
        pred_ens = sum(
            1
            for p in fp.values()
            if isinstance(p, dict) and p.get("selected_field_name")
        )
        ...
        llm_confs = [
            p["confidence"]
            for p in lp.values()
            if p and isinstance(p, dict) and "confidence" in p
        ]
        rag_confs = [
            p["confidence"]
            for p in rp.values()
        ...
        all_margins = [
            p.get("similarity_margin", 0)
            for ps in [lp.values(), rp.values()]
            for p in ps
        ...
        return {
            ...
            "field_counts": {
                "total_fields": total,
                "predicted_llm": pred_llm,
                "predicted_rag": pred_rag,
                "predicted_ensemble": pred_ens,
                "both_predicted": both,
                "same_predictions": same,
                "different_predictions": diff,
                "llm_only": llm_only,
                "rag_only": rag_only,
                "neither_predicted": neither,
            },
            "coverage": {
                "coverage_llm": cov_llm,
                "coverage_rag": cov_rag,
                "coverage_ensemble": cov_ens,
            },
            "agreement": {"agreement_rate": agree, "conflict_rate": conflict},
            "recovery": {"rag_recovery": rag_rec, "llm_recovery": llm_rec},
            "confidence": {
                "avg_conf_llm": _avg(llm_confs),
                "avg_conf_rag": _avg(rag_confs),
    ...
    def recalculate_accuracy_after_errors(
        self, user_id, session_id, pdf_id, errors
    ) -> dict[Any, Any] | None:
        base = f"predictions/{user_id}/{session_id}/{pdf_id}"
        metrics = self.storage.load_json(f"{base}/analysis/metrics_snapshot.json")
        final_preds = self.storage.load_json(
            f"{base}/predictions/final_predictions.json"
        )
        ...
        for error in errors:
            ...
            for _fid, fp in final_preds.get("final_predictions", {}).items():
                if isinstance(fp, dict) and fp.get("selected_field_name") == fn:
                    if fp.get("selected_from") == "llm":
                        err_llm += 1
                    elif fp.get("selected_from") == "rag":
                        err_rag += 1
        ...
        acc["accuracy_llm"] = _safe_div(
            fc["predicted_llm"] - acc["errors_llm"], fc["predicted_llm"]
        )
        acc["accuracy_rag"] = _safe_div(
            fc["predicted_rag"] - acc["errors_rag"], fc["predicted_rag"]
        )
        acc["accuracy_ensemble"] = _safe_div(
            fc["predicted_ensemble"] - acc["errors_ensemble"], fc["predicted_ensemble"]
        )
