...
class ProcessingPipeline:
    ...
    def run(
        ...
        self.storage.save_json(
            f"{base}/predictions/llm_predictions.json", llm_predictions
        )
        self.storage.save_json(
            f"{base}/predictions/final_predictions.json", final_predictions
        )
        ...
        self.storage.save_json(
            f"{base}/analysis/vector_update_summary.json", vector_summary
        )
    ...
    def _update_vectors(
        ...
        cat = pdf_cat.get("category")
        sub = pdf_cat.get("sub_category")
        doc = pdf_cat.get("document_type")
        updates = []
        updated = 0
        created = 0
            ...
            for fid in case_data["field_ids"]:
                ...
                if case_type == CASE_A:
                    ...
                    if rp_fid and rp_fid.get("vector_id"):
                        ...
                        updates.append(
                            {"field_id": fid, "action": "boosted", "reason": "CASE_A"}
                        )
                        updated += 1
                    ...
                    if sel == SOURCE_RAG:
                        ...
                        if rp_fid and rp_fid.get("vector_id"):
                            self.vector_store.update_confidence(
                                rp_fid["vector_id"], True
                            )
                            updates.append(
                                {
                                    "field_id": fid,
                                    "action": "boosted",
                                    "reason": "CASE_B RAG selected",
                                }
                            )
                            updated += 1
                    elif sel == SOURCE_LLM:
                        ...
                        if lp_fid and isinstance(lp_fid, dict):
                            ...
                            if ctx:
                                emb = self.embedding_backend.embed(
                                    self.embedding_backend.create_text_from_field(ctx)
                                )
                    ...
                    if rp_fid and rp_fid.get("vector_id"):
                        self.vector_store.update_confidence(rp_fid["vector_id"], True)
                        updates.append(
                            {"field_id": fid, "action": "boosted", "reason": "CASE_D"}
                        )
                        updated += 1
    ...
    def _field_context(self, field_id, user_id, session_id, pdf_id):
        data = self.storage.load_json(
            f"predictions/{user_id}/{session_id}/{pdf_id}/predictions/input.json"
        )
        if not data:
            return None
