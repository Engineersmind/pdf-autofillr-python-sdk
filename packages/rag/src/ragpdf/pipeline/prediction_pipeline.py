...
class PredictionPipeline:
    ...
    def run(
        ...
        self.storage.save_json(
            f"{meta_path}/submission_info.json",
            {
        ...
        avg_conf = (
            round(
                sum(r["confidence"] for r in results if r.get("matched")) / matched, 4
            )
            if matched
            else 0.0
        )
