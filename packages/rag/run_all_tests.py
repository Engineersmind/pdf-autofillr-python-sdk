...
check("vector_database.json",    Path("ragpdf_data/vectors/vector_database.json").exists(),
      "Run: python init_vector_db.py --source ragpdf_data/vectors/source/vector_base.json")
check("source/vector_base.json", Path("ragpdf_data/vectors/source/vector_base.json").exists())
check("sample_errors.json",      Path("ragpdf_data/input/sample_errors.json").exists())
check("llm_predictions_test",    Path("ragpdf_data/input/llm_predictions_test.json").exists(),
      "Copy to ragpdf_data/input/")
check("final_predictions_test",  Path("ragpdf_data/input/final_predictions_test.json").exists(),
      "Copy to ragpdf_data/input/")
...
if db_path.exists():
    ...
    vecs = db["vectors"]
    emb_ok   = [v for v in vecs if v.get("embedding") and any(x != 0 for x in v["embedding"])]
    emb_miss = len(vecs) - len(emb_ok)
    check(f"vector DB has {len(vecs)} vectors",      len(vecs) >= 10)
    check(f"all vectors embedded ({emb_miss} missing)", emb_miss == 0,
          f"{emb_miss} unembedded — run init_vector_db.py")
    dim = len(emb_ok[0]["embedding"]) if emb_ok else 0
    print(f"  {bold(str(len(vecs)))} vectors  |  dim={dim}  |  backend="
...
r = subprocess.run(
    ...
    capture_output=True, text=True, env=env_noop
)
...
'''
tmp = Path("_sanity_tmp.py"); tmp.write_text(sanity_code)
...
try:
    ...
    for res in results:
        ...
        print(f"  {res['vid']:<8} {res['exp']:<38} {res['got']:<38} {res['conf']:>7.4f}  {flag}")
    check(f"sanity: {correct}/5 correct", correct >= 4,
          "check OPENAI_API_KEY and that all vectors are embedded")
except Exception as e:
    print(yellow(r.stdout[-300:])); print(yellow(r.stderr[-200:]))
...
try:
    ...
    print(f"  total_vectors={tv}  total_submissions={info['summary']['total_submissions']}")
    check(f"system-info: total_vectors >= 10", tv >= 10, f"got {tv}")
except Exception as e:
...
if not src_path.exists():
    ...
    r = subprocess.run([
        sys.executable, "-m", "ragpdf.entrypoints.cli", "predict",
        "--user",     "test_u",
        "--session",  "test_s",
        "--pdf",      "test_p",
        "--fields",   str(tmp_fields),
        "--hash",     "testhash001",
        "--category", str(tmp_cat),
    ], capture_output=True, text=True)
    ...
    preds_path = Path("ragpdf_data/predictions/test_u/test_s/test_p/predictions/rag_predictions.json")
    sub_path   = Path("ragpdf_data/predictions/test_u/test_s/test_p/metadata/submission_info.json")
    ...
    if preds_path.exists():
        ...
        for fid, p in preds["predictions"].items():
            ...
            if p:
                ...
                if ok: correct += 1
                flag = green("PASS") if ok else red("FAIL")
                print(f"  {fid:<10} {p['predicted_field_name']:<40} {exp:<40} {p['confidence']:>7.4f}  {flag}")
            else:
                print(f"  {fid:<10} {'None':<40} {exp:<40} {'':>7}  {red('MISS')}")
        print(f"  {'-'*105}")
        check(f"predict: {correct}/{n_test} correct",   correct == n_test, f"{correct}/{n_test}")
        check(f"predict: avg_confidence >= 0.75",        avg_c >= 0.75,    f"{avg_c:.4f}")
        check(f"predict: all {n_test} fields matched",  matched == n_test, f"{matched}/{n_test}")
...
if not llm_p.exists() or not final_p.exists():
    print(yellow("  SKIP — copy llm_predictions_test.json + final_predictions_test.json"))
    print(yellow("         to ragpdf_data/input/"))
else:
    r = subprocess.run([sys.executable, "test_api2.py"], capture_output=True, text=True)
    print(r.stdout)
    if r.returncode != 0: print(yellow(r.stderr[-300:]))

    cc = Path("ragpdf_data/predictions/user_test/session_test/pdf_test/analysis/case_classification.json")
    ms = Path("ragpdf_data/predictions/user_test/session_test/pdf_test/analysis/metrics_snapshot.json")
    vu = Path("ragpdf_data/predictions/user_test/session_test/pdf_test/analysis/vector_update_summary.json")
...
'''
    tmp_a2 = Path("_api2_fb_tmp.py"); tmp_a2.write_text(api2_code)
    ...
    err_field = list(fp_dict.values())[0]["predicted_field_name"] if fp_dict else "investor_type"

    tmp_err = Path("_errors_tmp.json")
    tmp_err.write_text(json.dumps([{
    ...
    r = subprocess.run([
        sys.executable, "-m", "ragpdf.entrypoints.cli", "feedback",
        "--user",    "test_u",
        "--session", "test_s",
        "--pdf",     "test_p",
        "--errors",  str(tmp_err),
    ], capture_output=True, text=True, env=env_n)
    tmp_err.unlink(missing_ok=True)
    print(r.stdout)
    if r.returncode != 0: print(yellow(r.stderr[-200:]))

    fb_path = Path("ragpdf_data/predictions/test_u/test_s/test_p/errors/user_feedback_raw.jsonl")
...
for args, label, key in [
    ...
    (["--type", "pdf", "--user", "test_u", "--session", "test_s", "--pdf","test_p"],"pdf",      "submission_id"),
    (["--type", "category", "--category", "Private Markets"],                       "category", "time_series"),
]:
    r = subprocess.run(
    ...
    try:
        ...
        print(r.stdout[:200]); print(r.stderr[:100])
        check(f"metrics {label}: valid JSON", False)

# ─────────────────────────────────────────────────────────────────────────────
section("8. LAMBDA HANDLER — local")
# ─────────────────────────────────────────────────────────────────────────────
src_raw  = json.loads(Path("ragpdf_data/vectors/source/vector_base.json").read_text()) \
           if Path("ragpdf_data/vectors/source/vector_base.json").exists() else []
src_list = src_raw if isinstance(src_raw, list) else src_raw.get("vectors", [])
lf = src_list[0] if src_list else {
    "field_name":"investor_type","context":"Type of investor entity",
    "section_context":"Investor Classification","headers":["Investor Information"]
}
...
print(json.dumps(out))
'''
tmp = Path("_lambda_tmp.py"); tmp.write_text(lambda_code)
r   = subprocess.run([sys.executable, str(tmp)], capture_output=True, text=True)
tmp.unlink(missing_ok=True)
try:
    res = json.loads(r.stdout.strip().splitlines()[-1])
    print(f"  system_info:  {res['si_status']}  vectors={res['si_vectors']}")
    print(f"  predict:      {res['pred_status']}  matched={res['pred_matched']}/1  -> {res.get('pred_name','?')}")
    print(f"  bad key:      HTTP {res['bad_key']}")
    print(f"  bad api:      HTTP {res['bad_api']}")
    check("lambda: system_info success",      res["si_status"] == "success")
    check(f"lambda: total_vectors >= 10",     res["si_vectors"] >= 10)
    check("lambda: predict success 1/1",      res["pred_status"] == "success" and res["pred_matched"] == 1)
    check("lambda: predicted correct field",  res.get("pred_name") == lf["field_name"],
...
'''
bench = Path("_bench_tmp.py"); bench.write_text(bench_code)
print(f"  Embedding {len(src_list)} source vectors in batches of 50...")
print(f"  ({max(1, (len(src_list)+49)//50)} API calls  ~$0.001 total)\n")
r = subprocess.run([sys.executable, str(bench)], capture_output=True, text=True, timeout=180)
bench.unlink(missing_ok=True)

if r.stderr: print(yellow(f"  {r.stderr.strip()}"))
try:
    ...
    if fails:
        ...
        print(f"  {'-'*98}")
        for row in fails:
            print(f"  {row['vid']:<8} {row['expected']:<40} {row['got']:<40} {row['conf']:>7.4f}")
    else:
        print(f"  {green('Perfect — every vector maps back to itself.')}")

    check(f"benchmark: {correct}/{total} correct", correct == total,
          f"{total-correct} mismatches — check init_vector_db.py was run with same model")
    check(f"benchmark: accuracy >= 95%",           pct >= 95.0, f"got {pct:.1f}%")
except Exception as e:
    print(r.stdout[-400:]); print(r.stderr[-200:])
...
if FAIL == 0:
    ...
    print()
    print("Next:")
    print("  Server tests:    python test_api_server.py  (start server first in another terminal)")
