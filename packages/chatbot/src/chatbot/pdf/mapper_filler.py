...
class MapperPDFFiller(PDFFillerInterface):
    ...
    def __init__(self, mapper_api_url: Optional[str] = None,
                 mapper_api_key: Optional[str] = None,
                 url_prefix: Optional[str] = None,
                 config_dir: Optional[str] = None,
                 timeout: float = 300.0):
        self._api_url = (mapper_api_url or os.getenv("MAPPER_API_URL", "")).rstrip("/")
        self._api_key = mapper_api_key or os.getenv("MAPPER_API_KEY", "")
        self._url_prefix = (url_prefix if url_prefix is not None
    ...
    def prepare_document(self, pdf_path: str, investor_type: str,
        ...
        try:
            return impl.prepare_document(pdf_path, investor_type, session_dir=session_dir)
        except TypeError:
            return impl.prepare_document(pdf_path, investor_type)
    ...
    def _get_impl(self):
        if self._impl is not None:
            return self._impl
        if self._api_url:
            logger.info("MapperPDFFiller: HTTP mode -> %s%s", self._api_url, self._url_prefix)
            ...
            _cls = None if _submod_patched_out else sys.modules[__name__].__dict__.get("InProcessMapperFiller")
            if _cls is None:
                raise ImportError(
...
class _HttpMapperFiller:
    ...
    def prepare_document(self, pdf_path: str, investor_type: str) -> str:
        ...
        result = self._post("make-embed-file", {
            "pdf_path": pdf_path, "session_id": session_label, "investor_type": investor_type,
        })
        data = result.get("data", result)
        outputs = data.get("outputs", {})
        doc_id = (outputs.get("embedded_pdf") or data.get("embedded_pdf")
                  or data.get("embedded_pdf_path"))
        if not doc_id:
            logger.warning("_HttpMapperFiller: no embedded_pdf in response, falling back to pdf_path")
            doc_id = pdf_path
        return doc_id
