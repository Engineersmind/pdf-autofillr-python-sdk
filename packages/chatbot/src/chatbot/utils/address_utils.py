...
def is_address_field(field_path: str) -> tuple[bool, str | None, str | None]:
    """
    ...
    for pattern, addr_type in ADDRESS_PATTERNS:
        if pattern in field_path:
            ...
            for prefix in [
                "address_registered_",
                "address_mailing_",
                "address_jurisdiction_",
                "wiring_details_address_",
                "wiring_address_",
            ]:
...
def copy_registered_to_mailing(live_fill_flat: dict) -> dict:
    ...
    for key, value in list(live_fill_flat.items()):
        if "address_registered." in key:
            mailing_key = key.replace(
                "address_registered.", "address_mailing."
            ).replace("address_registered_", "address_mailing_")
            if mailing_key in live_fill_flat and (
                live_fill_flat[mailing_key] is None or live_fill_flat[mailing_key] == ""
            ):
